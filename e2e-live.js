...
(async () => {
  const phase = process.argv[2] || "love";
  if (phase !== "read") {
    const msgs = {
  ...
  for (const m of msgs) console.log(`[${m.senderType}] ${String(m.body).slice(0, 240)}`);

  const { PrismaClient } = require("@prisma/client");
  const p = new PrismaClient();
  const row = await p.chatConversation.findUnique({ where: { id: CONV_ID }, select: { aiMemory: true } });
  console.log("\n=== MEMORY NOW ===");
  console.log(row?.aiMemory || "(empty)");
  await p.$disconnect();
