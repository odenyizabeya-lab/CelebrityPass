...
export default async function CelebrityPage({ params }: Props) {
  ...
  return (
    <div>
      ...
      />
      {/* Cover */}
      {/* Cookie-gated /images/... routes must load directly in the browser: the
          next/image optimizer fetches them WITHOUT the fan cookie, the proxy redirects
          to /login and the optimizer returns 400 "not a valid image". Keep `unoptimized`
          on every local /images route or photos break again. */}
        ...
        <div className="relative z-10 -mt-24 flex flex-col gap-6 sm:flex-row sm:items-end">
          <div className="w-[320px] max-w-full shrink-0 sm:w-[450px] lg:w-[560px]">
            <div className="rounded-3xl bg-ink-900 p-2 shadow-2xl ring-4 ring-ink-900">
              <a
                href={`https://www.google.com/search?q=${encodeURIComponent(celebrity.name)}`}
                target="_blank"
                rel="noreferrer"
                className="block"
                aria-label={`Search Google for ${celebrity.name}`}
              >
